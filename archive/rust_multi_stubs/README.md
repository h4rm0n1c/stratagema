...
> Note: PNG icons and the example screenshot have been removed from version control to keep this archive PR-friendly. Retrieve the original PNGs from the `nvigneux/Helldivers-2-Stratagems-icons-svg` repository or from commit `9553938` in this repo, then place them back into `icons/` alongside this README before running the legacy build.
